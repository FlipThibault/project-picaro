    ...
    public class MapView : MonoBehaviour
    {
        ...
        public void SetAttainableNodes()
        {
            ...
            else
            {
                ...
                foreach (Vector2Int point in MapManager.getCurrentMap().path)
                {
        ...
        public virtual void SetLineColors()
        {
            ...
            if (MapManager.getCurrentMap().path.Count <= 1) return;

            for (int i = 0; i < MapManager.getCurrentMap().path.Count - 1; i++)
            {
                Vector2Int current = MapManager.getCurrentMap().path[i];
                Vector2Int next = MapManager.getCurrentMap().path[i + 1];
        ...
        protected virtual void SetOrientation()
        {
            ...
            float span = MapManager.getCurrentMap().DistanceBetweenFirstAndLastLayers();
            MapNode bossNode = MapNodes.FirstOrDefault(node => node.Node.nodeType == NodeType.Boss);
        ...
        {
            MapConfig config = GetConfig(MapManager.getCurrentMap().configName);
